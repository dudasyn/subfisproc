...
if ($method === 'POST') {
    ...
    $stmt = $pdo->prepare('SELECT u.id, u.name, u.role, u.department, u.sector_id, u.password, u.force_password_change, s.name as sector_name FROM users u LEFT JOIN sectors s ON u.sector_id = s.id WHERE (u.email = ? OR u.cpf = ?) AND u.active = 1');
